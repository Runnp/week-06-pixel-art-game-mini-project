# PUSH 17 — Fish Aquarium at Base Shelter
# Commit: "Push 17: Aquarium mechanic, feed fish, fish health tracker"
# This is the "care for small fish yet in aquarium" mechanic from the design doc.
#
# ════════════════════════════════════════════════════════════════
# FILE 1: res://scripts/world/aquarium.gd
# LOCATION: res://scripts/world/aquarium.gd
# Attach to: Node2D "Aquarium" placed inside Rustam's shelter
# ════════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════════
# FILE 2: res://scripts/world/fish_food_spot.gd
# LOCATION: res://scripts/world/fish_food_spot.gd
# Attach to: Area2D placed near water/reeds in Muynak level
# Player finds algae/dried food here to feed aquarium fish
# ════════════════════════════════════════════════════════════════

extends Area2D

const RESPAWN_TIME := 45.0   # seconds before food reappears
...
func _on_body_entered(body: Node2D) -> void:
	if not body.is_in_group("player") or _depleted:
		return

	_depleted = true
	Inventory.add_item("fish_food", 1)
	modulate.a = 0.3

	var hud := get_tree().get_first_node_in_group("hud")
	if hud:
		hud.show_prompt("Found fish food")
...
# ── SCENE STRUCTURE ───────────────────────────────────────────
# In Hometown.tscn (Rustam's shelter area) add:
#   Node2D (name: Aquarium, aquarium.gd)
#   ├── Sprite2D    (pixel art aquarium tank)
#   ├── Label       (name: Label)  shows fish count
#   └── Timer       (name: DecayTimer)
#
# In Muynak.tscn add 2-3 instances of:
#   Area2D (fish_food_spot.gd)
#   ├── Sprite2D    (algae/reed clump sprite)
